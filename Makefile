...
gen-schema: ## Regenerate frontend/event-schema from the pydantic event registry
	$(PY) backend/scripts/gen_event_schema.py
...
gen-api: ## Regenerate the web client's API types from the FastAPI OpenAPI document
	$(PY) backend/scripts/gen_openapi.py
	pnpm -F web gen-api

gen-api-check: ## Fail if the committed OpenAPI document is stale
	$(PY) backend/scripts/gen_openapi.py --check
