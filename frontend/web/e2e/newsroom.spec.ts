...
interface Article {
  ...
  languages: Record<
    ...
    {
      title: string;
      blocks: { type: string; text: string; claim_ids: string[] }[];
    }
  >;
}

test("a story from the feeds to the public site, and from the office to its draft", async ({
  page,
}, info) => {
  ...
  const mode = await page
    .locator("[data-office-mode]")
    .getAttribute("data-office-mode");
  if (mode === "3d") {
  ...
  await expect(panel(page)).toContainText(storyTitle);
  ...
  const [, articleId, version] = href.match(
    /\/newsroom\/articles\/([0-9a-f-]+)\?version=(\d+)/,
  )!;
  const article = await api<Article>(
    page,
    `/api/articles/${articleId}?version=${version}`,
  );
  ...
  await expect(
    page.locator("#fact-check").getByText("通過", { exact: true }).first(),
  ).toBeVisible();
  await page.screenshot({
    path: info.outputPath("draft-page.png"),
    fullPage: true,
  });
  ...
  await expect
    .poll(
      async () =>
        (await api<Article>(page, `/api/articles/${articleId}`)).public_urls[
          "en"
        ] ?? null,
      {
        timeout: 60_000,
  ...
  await expect
    .poll(
      async () => {
        const detail = await api<{ distributions: { channel: string }[] }>(
          page,
  ...
  await expect(page).toHaveURL(
    new RegExp(`/news/en/articles/${article.slug}$`),
  );
  await expect(page.getByRole("heading", { level: 1 })).toHaveText(
    article.languages["en"]!.title,
  );
