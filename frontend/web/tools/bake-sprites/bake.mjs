...
const OUT_PEOPLE = join(WEB, "src/office3d/fallback/art/baked-people.ts");

async function bundle(into, entry = "people.ts") {
  await build({
...
/**
 * The page is served from a made-up origin rather than opened as a file: a character's GLB names
 * its texture by a relative URL (``Textures/colormap.png``), and a browser will not let a file://
 * page fetch its neighbours. Nothing leaves the machine — every request is answered from disk here.
 */
const ORIGIN = "http://bake.local";
const PUBLIC = join(WEB, "public");
const TYPES = { ".html": "text/html", ".js": "text/javascript", ".glb": "model/gltf-binary", ".png": "image/png" };

async function serve(page, into) {
  await page.route(`${ORIGIN}/**`, async (route) => {
    const path = decodeURIComponent(new URL(route.request().url()).pathname);
    const file = path.startsWith("/models/") ? join(PUBLIC, path) : join(into, path);
    if (!file.startsWith(PUBLIC) && !file.startsWith(into)) return route.fulfill({ status: 403 });
    try {
...
async function renderInABrowser(into) {
  ...
  try {
    ...
    await serve(page, into);
    await page.goto(`${ORIGIN}/bake.html`);
    const baked = await page.evaluate(() => window.bakeAll());
    const people = await page.evaluate(() => window.bakePeople());
...
/** ``aaaab..`` -> ``4ab2.``: a run's length, then its character; a single character stands alone. */
function rle(line) {
  let out = "";
  for (let i = 0; i < line.length; ) {
    let j = i;
    while (j < line.length && line[j] === line[i]) j++;
    out += (j - i > 1 ? String(j - i) : "") + line[i];
    i = j;
  }
  return out;
}

function emitPiece(s) {
...
${keys}
    ...
    rows: [
${s.rows.map((line) => `      ${quote(rle(line))},`).join("\n")}
    ],
    shadow: [
${s.shadow.map((line) => `      ${quote(rle(line))},`).join("\n")}
    ],
  },`;
}

function emitPeople(people, walkFrames) {
  return `// Generated by tools/bake-sprites — do not edit by hand (D-027).
//
...
export const WALK_FRAMES = ${walkFrames};

export const PEOPLE: Record<string, EncodedPiece> = {
${people.map(emitPiece).join("\n")}
};
`;
}

function emit(sprites, placed, backdrop) {
  const pieces = sprites.map(emitPiece).join("\n");

  return `// Generated by tools/bake-sprites — do not edit by hand (D-026, D-027).
//
...
export const BACKDROP_KEY = ${quote(backdrop)};
...
${placed.map((p) => `  { bake: ${quote(p.bake)}, at: [${p.at[0]}, ${p.at[1]}]${p.seat ? `, seat: ${quote(p.seat)}` : ""} },`).join("\n")}
];
...
async function preview(into, dir) {
  ...
  try {
    ...
    const floors = await page.evaluate(() => window.previewFloors());
    floors["people.png"] = await page.evaluate(() => window.previewPeople());
    if (problems.length) throw new Error(`the browser complained:\n  ${problems.join("\n  ")}`);
    const pictures = [...sheets.map((url, i) => [names[i], url]), ...Object.entries(floors)];
    for (const [name, url] of pictures) {
      await writeFile(join(dir, name), Buffer.from(url.split(",")[1], "base64"));
...
try {
  ...
  if (previewAt !== -1) {
    ...
    await bundle(into);
    const baked = await renderInABrowser(into);
    const sprites = baked.sprites.map(trim);
    const people = baked.people.sprites.map(trim);
    const generated = emit(sprites, baked.placed, baked.backdrop);
    const generatedPeople = emitPeople(people, baked.people.walkFrames);
    if (process.argv.includes("--check")) {
      const committed = await readFile(OUT, "utf8").catch(() => "");
      const committedPeople = await readFile(OUT_PEOPLE, "utf8").catch(() => "");
      if (committed !== generated || committedPeople !== generatedPeople) {
        throw new Error("the baked sprites are stale: run `pnpm -F web bake-sprites`");
      }
      console.log("baked sprites are up to date");
    } else {
      await writeFile(OUT, generated);
      await writeFile(OUT_PEOPLE, generatedPeople);
      const figures = people.reduce((n, s) => n + s.w * s.h, 0);
      console.log(`baked ${people.length} figures, ${figures} pixels -> ${OUT_PEOPLE.replace(`${WEB}/`, "")}`);
