...
- **公開站**（T-515，讀者看的網站，不需要權杖）：`/zh-TW`、`/en` 是最新報導，`/{語言}/articles/{slug}` 是文章（發布出去的那一版、該語言；另一種語言的連結、資料來源、`hreflang`）。沒發布或沒有該語言時是 404。讀者打開文章記一次瀏覽、讀到結尾記一次讀完（`POST /api/analytics/beacon`）；瀏覽器每天產生一個隨機 ID 存在 localStorage，不送 IP 或任何可辨識讀者的資料，同一 ID 同一天同一篇同一語言只算一次。
