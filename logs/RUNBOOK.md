...
- **新聞室**（T-517）：Dashboard 右上角「新聞室」。三個分頁：**題材**（`/newsroom/stories`，依狀態篩選；每則題材的線索、擷取的證據、主張〔展開可看每段引文在原文中的位置〕、工作流程的時間軸；新發現或已選定的題材有「開始製作」按鈕，會選定並啟動整條新聞流程）、**文章**（`/newsroom/articles`；每篇文章可切換版本 `?version=N` 與語言，段落後面的 [1][2] 是引用的主張，下方是事實查核、發布紀錄、讀者統計與時間軸；已發布的有公開頁連結）、**來源**（`/newsroom/sources`，列出來源與帶來的項目數，可新增 RSS、網址清單或搜尋來源）。3D 辦公室裡代理面板的連結（題材與來源、草稿、事實查核、發布紀錄）都連到這些頁面。
- **公開站**（T-515，讀者看的網站，不需要權杖）：`/news/zh-TW`、`/news/en` 是最新報導，`/news/{語言}/articles/{slug}` 是文章（發布出去的那一版、該語言；另一種語言的連結、資料來源、`hreflang`）。沒發布或沒有該語言時是 404。讀者打開文章記一次瀏覽、讀到結尾記一次讀完（`POST /api/analytics/beacon`）；瀏覽器每天產生一個隨機 ID 存在 localStorage，不送 IP 或任何可辨識讀者的資料，同一 ID 同一天同一篇同一語言只算一次。
  - `SITE_COMPANY`（選填，公司的 slug）：只列這家公司的報導；不設就列出所有公司已發布的報導。
  - `API_INTERNAL_URL`（選填）：公開站在伺服器端讀 API 用的位址；預設同 `NEXT_PUBLIC_API_URL`。Docker 內設為 `http://api:8000`（瀏覽器用的 `localhost:8000` 在容器裡連不到）。
...
**題材（Story，T-504）**：輪詢到的項目每 5 分鐘（比輪詢晚 2 分鐘）自動分群成「題材」：同一網址、或內容夠像（`NEWSROOM_STORY_MATCH_THRESHOLD`，預設 0.65；這是新聞室自己的設定，不在核心的 `Settings` 裡）的項目歸到同一個題材，不像的開新題材，時間軸顯示「發現題材：標題・分數」。分數看幾個不同來源報導、多新、來源信任度。已經寫過、被忽略或放棄的題材也會吸收同一件事的新報導，不會重複出現。目前中英文報導同一件事時會是兩個題材（跨語言合併會誤併相關報導）。換 embedding 模型時要重新確認門檻（`tests/newsroom/test_embed_live.py` 會檢查）。
...
階段 3 的驗收：兩個分頁同步、API 停 20 秒再啟動後自動恢復；階段 4 的 3D 辦公室；**階段 5 的新聞室**（`e2e/newsroom.spec.ts`，T-520：從示範新聞室的 fixture 來源選一則題材按「開始製作」→ 工作程序跑到核准 → 在 3D 辦公室點寫手、從面板的連結進入草稿頁，內容與 API 一致 → 在審批收件匣核准 → 公開站中英文都能讀）。測試資料庫除了 echo 公司，也會建立示範新聞室並先讀取來源、分群成題材（`seed_newsroom.py --gather`）。只跑新聞室：`pnpm -F web exec playwright test newsroom`。需要資料庫在跑（`make dev`）與 Python 虛擬環境；測試會自己啟動另一組 API（埠 8100）、工作程序與前端（埠 3100，正式建置，放在 `.next-e2e`），用獨立的資料庫 `<開發資料庫>_e2e`（每次重建），模型固定用模擬的，**不讀 `.env`**，所以不會用到真的金鑰，也不會動到開發資料。開著的開發伺服器（8000、3000）不受影響，但 8100 與 3100 要空著。
