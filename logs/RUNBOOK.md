...
- **3D 辦公室**（階段 4 進行中）：`/office`（Dashboard 右上角「辦公室」）。標頭可切換「自動 / 3D / 2D」，也可以直接在網址加 `?view=3d` 或 `?view=2d`。沒有 WebGL 2 的瀏覽器一律顯示 2D 看板；手機寬度預設 2D。目前 3D 只有地板，房間、桌椅與人物陸續加入（T-402 起）。若畫面顯示「3D 暫停」，是瀏覽器收回了繪圖資源，按「重新建立 3D」即可。
