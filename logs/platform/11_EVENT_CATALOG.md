...
| Revenue（T-701、D-024） | CUSTOMER_ACQUIRED, CUSTOMER_CHURNED, CUSTOMER_RETURNED, MEMBERSHIP_GRANTED, MEMBERSHIP_EXPIRED, PAYMENT_RECEIVED | 是 | 營收投影、客戶價值 | `company/customers`、`company/memberships`（由金流的付款通知呼叫；到期由每日 cycle 處理） |
...
| Governance | APPROVAL_REQUESTED, APPROVAL_APPROVED, APPROVAL_REJECTED, APPROVAL_RETURNED（D-044）, APPROVAL_EXPIRED, POLICY_DENIED | 是 | 審計 | Approvals / PolicyEngine |
| Model | （無 event；`model_calls` 表） | — | — | Gateway |
| Scheduler | SCHEDULE_FIRED | 是（可歸檔） | — | Scheduler |
| Reporting | KPI_SNAPSHOT_CREATED | 是 | — | Reporting |
| Newsroom | SOURCE_POLLED, SOURCE_ITEM_DISCOVERED(可歸檔), SOURCE_PAUSED, EVIDENCE_CAPTURED, STORY_DISCOVERED, STORY_SELECTED, STORY_DROPPED, CLAIM_CREATED, CLAIM_VERIFIED, CLAIM_REJECTED, ARTICLE_CREATED, ARTICLE_REVISION_REQUESTED, ARTICLE_REVIEWED, ARTICLE_APPROVED, ARTICLE_REJECTED, ARTICLE_RETURNED, ARTICLE_PUBLISHED, ARTICLE_UNPUBLISHED, ARTICLE_REPUBLISHED（後三者 D-044）, ARTICLE_REVISION_STARTED, ARTICLE_REVISION_DROPPED（D-045）, DISTRIBUTION_CREATED, ANALYTICS_DAILY_UPDATED | 是 | 內容 pipeline 投影、溯源 | Newsroom tools / services |
