...
| T-519 | `tests/e2e/test_newsroom_real.py`（真模型跑整條新聞線，工具仍離線 fixture；自己的資料庫、45 分鐘上限、失敗的一步等重試）；runner 在等模型回覆時持續續租（`_keeping_the_lease`，單次呼叫可能比租約長）；分析師的關鍵數字規則改成會說明怎麼修；結論見開發紀錄（免費端點太慢，整條線未跑完） |
