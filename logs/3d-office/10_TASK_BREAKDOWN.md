...
| T-517 | 後端：`domains/newsroom/admin.py`（題材列表 / 詳情〔線索、證據、主張與引文前後文、工作流程〕、文章列表 / 詳情〔各版本、某版各語言內容與主張、查核、發布紀錄、每日讀者〕、來源列表）、`api/routers/newsroom.py`（`GET /api/companies/{id}/stories`、`GET /api/stories/{id}`、`POST /api/stories/{id}/start`、`GET /api/companies/{id}/articles`、`GET /api/articles/{id}?version=`、`GET/POST /api/companies/{id}/sources`）、`/api/events?correlation_id=`；前端：`(admin)/newsroom/{stories,stories/[storyId],articles,articles/[articleId],sources}`、`features/newsroom/`（`model`、`parts`〔標籤、導覽、主張清單、時間軸〕、五個 View、`pages.tsx`）、查詢與失效（新聞室事件 → `["newsroom"]`、工作流程事件 → 該流程的時間軸）、Dashboard「新聞室」連結 |
| 階段 5 收尾 | `demo.editor_fails` 與 `test_an_editor_that_never_decides_fails_visibly`（AC-9 的失敗全程）；`e2e/office.spec.ts` 的 AC-1 測試（即時指示、就座時間）；`11_MVP_ACCEPTANCE.md` 與開發紀錄的驗收總結 |
| T-520 | `frontend/web/e2e/newsroom.spec.ts`（題材頁開始製作 → 等核准 → 3D 辦公室點寫手 → 面板連結 → 草稿頁內容與 API 一致、主張標記與引文、查核通過 → 收件匣核准 → 發布與社群草稿 → 公開站中英文）；`e2e/stack.ts`、`scripts/e2e_prepare.py`（加示範新聞室、`seed_newsroom.py --gather`、工作程序處理兩家公司、fixture 工具與 fake embedding）；**活動事件帶 `links`**（`AGENT_THINKING / WORKING / REVIEWING / RUN_COMPLETED`，`runtime/activity/service.py` 寫入事件本身）；辦公室與面板的「等待人工審批 / 系統」；時間軸時間不換行 |
| T-519 | `tests/e2e/test_newsroom_real.py`（真模型跑整條新聞線，工具仍離線 fixture；自己的資料庫、45 分鐘上限、失敗的一步等重試）；runner 在等模型回覆時持續續租（`_keeping_the_lease`，單次呼叫可能比租約長）；分析師的關鍵數字規則改成會說明怎麼修；結論見開發紀錄（免費端點太慢，整條線未跑完：第二次以 4 小時上限重跑，研究員再次成功，分析師三次嘗試兩次 504、一次回覆被 `max_tokens` 截斷 → 分析師的 `max_output_tokens` 4096 改 8192） |
