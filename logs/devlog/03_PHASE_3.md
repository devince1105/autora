...
## 階段 3 驗收（2026-09-18，通過）

路線圖的驗收條件逐項對應到 T-315 的兩個瀏覽器測試（本機與 CI 都執行）：

| 驗收條件 | 怎麼驗證 | 結果 |
|---|---|---|
| 兩個分頁開 Dashboard，啟動 EchoWorkflow，代理卡片同步變化 | 兩個 Dashboard 分頁：分析師忙碌時兩邊顯示同一個狀態（思考或工作中），結束時所有卡片狀態相同；另兩個時間軸分頁顯示的事件與伺服器完全相同 | ✅ |
| 關閉 API 20 秒再啟動，分頁自動恢復 | 以 SIGKILL 停掉 API，停機滿 20 秒（期間工作程序把工作流程跑完）再啟動；不重新整理頁面，30 秒內回到「即時」 | ✅ |
| `last_seq` 與伺服器一致 | 頁面最新一則事件的 seq 等於伺服器的最大 seq，且從頁面最舊一則到最新沒有缺號、沒有重複 | ✅ |
| Trace 無缺漏 | 停機期間完成的寫手執行：軌跡頁的每一列與 `GET /api/runs/{id}/trace` 完全相同 | ✅ |

- 路線圖的測試項目：閘道（backlog、snapshot、溢出、輪詢備援，T-303）、投影契約（T-302）、reducer 排序 / 去重（T-305）、client 重連（T-306）、Playwright 斷線恢復且不重複（T-315）都已完成，並在 CI 執行。
- **階段 4 的進入條件**：「階段 3 驗收 + 投影契約測試在 CI」——契約測試（`tests/realtime/test_contract.py`）在 CI 的 python 工作中執行，已滿足。

---

## T-315 · 階段 3 端到端測試（Playwright）

### 做了什麼
- `frontend/web/e2e/realtime.spec.ts`：兩個測試（上面驗收表的四項）。
- `frontend/web/e2e/stack.ts`：測試自己啟動與停止後端（API 在 8100、工作程序），可以在測試中途殺掉 API 再啟動；等待工作程序的日誌判斷「停機期間已跑完」。
- `backend/scripts/e2e_prepare.py`：使用 `<開發資料庫>_e2e`（不存在就建立），清空、套用遷移、建立 echo 示範公司，輸出連線資訊。
- `playwright.config.ts`：前端用正式建置（`next build` + `next start`，埠 3100），建置目錄 `.next-e2e`（`next.config.ts` 的 `NEXT_DIST_DIR`），不會和開發伺服器的 `.next` 互相干擾；本機用已安裝的 Google Chrome，CI 用 Playwright 的 Chromium。
- 隔離：所有後端程序設定 `AUTORA_ENV_FILE=/dev/null`，**不讀開發者的 `.env`**，模型固定為模擬、權杖是測試專用的，開發資料庫不會被動到。
- 指令：`make e2e`（= `pnpm -F web e2e`）；CI 新增 `e2e` 工作（Postgres、Python、Node、Chromium），失敗時上傳追蹤檔。
- 時間軸是「每個事件恰好一次」的見證：每列以 seq 為鍵，直接和 `GET /api/events` 比對。

### 過程中的問題
- 第一次執行時，e2e 的建置在型別檢查失敗，原因是**開發伺服器自己產生的 `.next/dev/types/validator.ts` 內容壞了**（19:10 寫壞，早於 e2e 建置），而共用的 tsconfig 會把它納入。處理：重啟開發伺服器讓它重新產生；e2e 的建置不做型別檢查（型別檢查由 `pnpm typecheck` 負責，e2e 建置不應依賴開發伺服器的產物）。
- 初版只用時間軸驗證、API 只停約 8 秒；對照路線圖的驗收文字後補上：兩個 **Dashboard** 分頁的代理卡片、停機**滿 20 秒**、最新 seq 等於伺服器、停機期間那次執行的**軌跡頁**完整。

### 驗證
- 本機 `make e2e`：2 個通過；再以 `--repeat-each 2` 連跑 4 次全部通過（不會時好時壞）。
- **破壞測試**：暫時讓前端忽略重連後的 backlog（停機期間的事件），停機測試就失敗（頁面缺少 WORKFLOW_RUN_COMPLETED），還原後通過——測試確實能抓到漏事件。
- web 97 個、event-schema 8 個單元測試；`typecheck`、`lint`、`next build`、`gen-api:check`、`make lint-py` 以結束碼確認通過。
- 測試結束後沒有殘留的程序或占用的埠（8100、3100）。
...
### 做了什麼
- 新頁面 **`/approvals`**：分頁「待審批 / 已核准 / 已駁回 / 已過期」（`GET /api/approvals?state=`）。每張卡片顯示類型、動作、提出者（代理名稱）、摘要、將執行的參數、已等待多久、多久後過期（一小時內以警告色），並連到執行軌跡與任務；已決定的顯示誰、何時、理由。
- **決議**：填選填的理由，按「核准」或「駁回」→ `POST /api/approvals/{id}/decide`。**列表不在前端直接改**：卡片先顯示「已送出，等待更新…」，由決議產生的 APPROVAL_* 事件讓 `["approvals", 公司]` 失效後重取（T-308 的對應表）。即時連線不在線時不會有事件，因此改為立即重取；已被處理（409）或不存在（404）會提示並重取；其他錯誤保留卡片與按鈕。
- Dashboard 右上角加上「審批收件匣」連結與待審批數（AC-10 的 Pending approvals）。
- **開發用的審批開關**：原本的 EchoWorkflow 不會產生任何審批，收件匣沒有真實資料可看。`seed_echo.py --approval on` 設定公司政策的收緊覆寫（`echo_note` / `writer` = `needs_approval`），寫手就會停下等人；`--approval off` 關閉。走的是既有的政策引擎與執行器路徑，沒有加測試專用的程式路徑。

### 真實環境測試
開關打開後啟動一次 EchoWorkflow，全程用真實程序（API、工作程序、模擬模型）：
- 研究員、分析師完成後，寫手的執行 `suspended NeedsApproval`，收件匣出現一筆（存成測試資料 `__fixtures__/pending.json`）。
- 用收件匣會送的同一個請求核准 → 回傳 APPROVED、決定者 `human/operator`；再送一次得到 **409**；待審批列表變成 0、已核准 1。
- 寫手的軌跡：`APPROVAL_REQUESTED → TASK_WAITING → AGENT_WAITING → APPROVAL_APPROVED → TASK_STARTED → … → TOOL_COMPLETED → TASK_SUCCEEDED → AGENT_RUN_COMPLETED`，同一次嘗試接著完成。
- 測試後已把開關關回 `off`。途中發現上一段對話啟動的工作程序仍在執行，接手了恢復後的執行，多開的一個已停止。

### 驗證
- `approvals.test.tsx`：9 個通過（真實審批資料）：卡片內容、已決定的顯示、APPROVAL_* 事件讓列表失效、核准後等待事件再更新、駁回帶理由、連線中斷時立即重取、409 提示並重取、其他錯誤、分頁；`dashboard.test.tsx` 新增待審批連結 1 個。
- web 共 97 個測試、event-schema 8 個通過；`typecheck`、`lint`、`next build`（新路由 `/approvals`）、`gen-api:check`、`make lint-py`、`make gen-api-check` 以結束碼確認通過。
- 瀏覽器窗格：`/approvals` 編譯無錯誤、主控台無錯誤，停在權杖畫面；登入後的畫面由元件測試與上面的真實 API 流程涵蓋。
...
### 做了什麼
- 新頁面 **`/timeline`**（權杖畫面之後；`?company=<ID>` 指定公司，與 Dashboard 相同），Dashboard 右上角有「事件時間軸」連結，時間軸頁可回 Dashboard，連結保留所選公司。
- 資料就是即時 store 的最近事件（hydrate 時伺服器給最近 100 則，之後隨串流累積，**最多保留 500 則**，最舊的先移出）。依 seq **由新到舊**排列，每則顯示時間、seq、誰（代理名稱，或系統 / 人員）、易讀的說明、原始類型名稱，有執行的連到 `/trace/<run_id>`、有任務的連到 `/tasks/<task_id>`，原始資料可展開。
- **暫停**：畫面凍結在按下時的內容，store 照常套用事件（不會漏），狀態列顯示暫停期間進來幾則（套用目前篩選）；按「繼續」一次顯示全部。
- **篩選**：依代理、依事件類型（可複選，兩者同時使用時取交集），選項是公司的代理與緩衝區裡出現過的類型；篩選也作用在暫停中的畫面。暫停與篩選存在 UI store（T-307），之後 3D 辦公室也能共用。
- 事件的中文說明從軌跡檢視器抽成共用的 `features/events/describe.ts`（軌跡和時間軸同一種事件讀起來一樣），並補上工作流程、代理建立 / 暫停 / 恢復、工具被拒、收支、預算、排程等類型。
- 選公司的邏輯從 Dashboard 抽成 `CompanyScope`（載入中、錯誤、找不到公司、401 回權杖畫面），連線狀態的計算抽成 `connectionModel`，兩頁共用。

### 驗證
- `timeline.test.tsx`：8 個通過，**用 T-302 契約測試產生的真實執行歷史**（148 個事件）：由新到舊且數量等於緩衝區、代理名稱、依代理 / 類型 / 兩者篩選、篩選選項、**套用 600 則後只留最新 500 則**、暫停後畫面不變且計算新事件數、繼續後顯示全部、清除篩選、執行與任務連結、從 store 設定的篩選也生效。
- web 共 87 個測試、event-schema 8 個通過；`typecheck`、`lint`、`next build`（新路由 `/timeline`）、`gen-api:check` 以結束碼確認通過（本次沒有改後端）。
- 瀏覽器窗格：`/timeline` 與重構後的 `/dashboard` 在開發伺服器編譯無錯誤、主控台無錯誤，停在權杖畫面；登入後的畫面由元件測試涵蓋。
...
### 做了什麼
- 新增兩個頁面（都在權杖畫面之後）：
  - **`/trace/{runId}`**：一次執行的完整軌跡。標頭有任務名稱、嘗試次數、狀態、費用、事件 / 步驟 / 工具呼叫數，以及回到任務的連結；下方依序列出事件。
  - **`/tasks/{taskId}`**：任務的輸入、輸出與**每一次嘗試**，各連到自己的軌跡（重試時可比較不同嘗試）。
- 進入點：代理面板「本次執行」旁的「完整軌跡」連結，以及任務頁。
- `features/trace-viewer/model.ts`（純函式）：
  - **每一列都是真實的事件或步驟**，前端不自行合成。已知的事件類型顯示成易讀的一行（例如「工具完成・echo_note・12 ms・wrote note…・產出 1」）；
  - **不認得的事件類型照樣顯示**：標題就是類型名稱，加上「未知類型」標記，原始資料預設展開，頁首也列出這些類型（伺服器比前端新、或其他領域的事件都不會消失）；
  - 沒有事件指向的步驟（例如 evaluate）依時間插入；好幾個事件共用同一步（WORKING、TOOL_CALLED、TOOL_COMPLETED）時，步驟只在第一個事件下顯示一次。
- 每一步可按「顯示完整提示與回應」，這時才向 `GET /api/runs/{id}/steps/{seq}/blob` 取完整內容（大小可能很大，不預先載入）；已知事件的原始資料可展開。
- 頁面連上公司的即時串流，執行中的軌跡會隨事件自動重取（沿用 T-308 的事件對應表）。

### 與設計不同
- 任務說「可由任務 / 文章（correlation）進入」：**任務入口已完成；文章入口要等階段 5 有文章頁（T-517）**，屆時由文章的 correlation 找到執行再連過來。
- OpenAPI 裡有兩個都叫 `TaskOut` 的模型（工作流程與任務明細），產生器因此輸出帶模組路徑的醜名稱。改名為 `WorkflowTaskOut` 與 `TaskDetailOut`，重新產生型別；API 的 JSON 內容沒有改變。

### 驗證
- `trace-viewer.test.tsx`：7 個通過，**用真實軌跡當測試資料**（開發環境 echo 分析師的一次執行，10 個事件、4 個步驟，存成 `__fixtures__/echo-trace.json`）：事件順序與數量完全對應、evaluate 步驟依時間插入、共用的步驟只顯示一次、已知類型的文字、未知類型保留原始資料並列在頁首、任務連結、按需載入完整提示、原始資料展開。
- web 共 79 個測試、event-schema 8 個通過；`typecheck`、`lint`、`next build`（兩個新路由）、`gen-api:check`、`make lint-py`、`make gen-api-check` 以結束碼確認通過；後端 654 個測試通過（改名後）。
- 瀏覽器窗格：`/trace/{runId}` 在開發伺服器編譯無錯誤、主控台無錯誤，顯示權杖畫面（頁面標題「Trace · Autora」）。輸入權杖後的畫面我沒有操作（不在欄位中輸入權杖），內容由元件測試涵蓋。
...
| 提交 | 日期 | 內容 | 持續整合 |
|---|---|---|---|
| `cfd1fd0` | 2026-09-18 | T-315 Playwright 端到端測試；階段 3 驗收通過 | ✅ 執行編號 `35339386413`（e2e 2 分 17 秒，2 個測試通過；python 1 分 58 秒、web 41 秒） |
| `16ec403` | 2026-09-18 | T-313 審批收件匣；`seed_echo.py --approval` | ✅ 執行編號 `35337505433`（python 1 分 49 秒、web 46 秒） |
| `f83021c` | 2026-09-18 | T-312 事件時間軸；事件說明、選公司、連線狀態抽成共用 | ✅ 執行編號 `35336385564`（python 1 分 48 秒、web 50 秒） |
| `a9e9f43` | 2026-09-18 | T-311 Trace Viewer；OpenAPI 的 `TaskOut` 改名 | ✅ 執行編號 `35335622135`（python 2 分 22 秒、web 41 秒） |
| `205e6a5` | 2026-09-18 | T-310 代理卡片與詳細面板 | ✅ 執行編號 `35332414679`（python 1 分 51 秒、web 37 秒） |
| `1ea8a98`、`7dcc26f` | 2026-09-18 | D-008 3D 人物風格（CC0 低多邊形；參考《動物森友會》與《Good Job!》） | — |
