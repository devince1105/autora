...
| 編號 | 內容 | 為什麼還沒做 |
|---|---|---|
| 尾-1 | **重新量 AC-S7 的 soak**：現有數字（FPS、heap、WS p95）是 T-600 第三～五批之前量的，部門分區、事業色帶、房間隔離都改過場景 | 數字沒有隨改動重量，等於沒有保證；`office-soak.spec.ts` 已經有，要的是跑一次並把新數字寫進 `11_MVP_ACCEPTANCE.md` |
| 尾-2 | **真的把 `domains/newsroom` 刪掉一次**，跑 runtime / company / realtime 的測試（AC-S5 的最後一句） | `lint-imports` 保證的是「沒有 import」，不等於「拿掉之後還會動」。這件事只能做一次實驗來答，做完把結果寫下來、把目錄還原 |
| 尾-3 | **AC-13 的端到端測試**：預算調到極低 → model call 被拒 → run ABORTED → `BUDGET_EXHAUSTED` 進 timeline → 加預算後任務自己回到佇列 | 各段都有單元測試（`guard.py`、補-3 的放回佇列），但沒有一個測試把整條走完；AC-13 要的正是整條 |
