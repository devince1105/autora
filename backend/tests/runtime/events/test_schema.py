...
EXPECTED_RUNTIME = {
    "AGENT_CREATED", "AGENT_PAUSED", "AGENT_RESUMED", "AGENT_RETIRED", "AGENT_IDLE",
    "AGENT_RUN_STARTED",
    ...
    "WORKFLOW_RUN_CANCELLED", "WORKFLOW_RUN_EXTENDED",
    "TASK_CREATED", "TASK_READY", "TASK_STARTED", "TASK_WAITING", "TASK_SUCCEEDED",
    "TASK_FAILED", "TASK_CANCELLED", "TASK_BLOCKED",
    "APPROVAL_REQUESTED", "APPROVAL_APPROVED", "APPROVAL_REJECTED", "APPROVAL_RETURNED",
    "APPROVAL_EXPIRED",
    "POLICY_DENIED", "SCHEDULE_FIRED", "BUDGET_EXHAUSTED",
}  # fmt: skip

# logs/platform/11_EVENT_CATALOG.md company groups. Memberships and payments are here, not in a
# business domain: every industry is paid the same way (T-701, D-024).
EXPECTED_COMPANY = {
    ...
    "CUSTOMER_ACQUIRED", "CUSTOMER_CHURNED",  # who pays the company (T-612)
    "CUSTOMER_RETURNED", "MEMBERSHIP_GRANTED", "MEMBERSHIP_EXPIRED", "PAYMENT_RECEIVED",  # D-024
    "KPI_SNAPSHOT_CREATED",
    "BUSINESS_UNIT_CREATED", "BUSINESS_UNIT_PAUSED", "BUSINESS_UNIT_SCALED",
    "BUSINESS_UNIT_WOUND_DOWN", "DEPARTMENT_CREATED", "PRODUCT_CREATED",
    "AGENT_ASSIGNED", "BUDGET_WARNING",
    # the business loop (T-611): what the company might do, and what it decided
    "OPPORTUNITY_DISCOVERED", "OPPORTUNITY_SIGNAL_RECORDED", "OPPORTUNITY_SCORED",
    "OPPORTUNITY_ADVANCED", "OPPORTUNITY_REJECTED", "OPPORTUNITY_EXPIRED",
    "PROPOSAL_DRAFTED", "PROPOSAL_SUBMITTED", "PROPOSAL_DECIDED", "PROPOSAL_SUPERSEDED",
}  # fmt: skip
